...
test_that("Compilation can handle multiple outputs", {
  ...
  setwd(tempdir())
  random_factory(include_examples = TRUE)
  log_file <- write.csv(data.frame(start = TRUE), ".compile_log.csv",
                              stringsAsFactors = FALSE)
  
  compile_report(list_reports(pattern = "foo")[1], quiet = TRUE, params = list("other" = "test"))
  
  log_file <- read.csv(".compile_log.csv",
                              stringsAsFactors = FALSE)
